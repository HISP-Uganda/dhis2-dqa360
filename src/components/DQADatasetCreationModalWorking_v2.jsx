...
export const DQADatasetCreationModal = ({
    ...
    const createDataElements = async () => {
        ...
        // Get category combo to use (default or from selected data elements)
        let categoryCombo = { id: 'bjDvmb4bfuf' } // Default
        
        if (selectedDataElements.length > 0 && selectedDataElements[0].categoryCombo) {
            try {
                const sourceCombo = await getCategoryComboFromSource(selectedDataElements[0].categoryCombo.id)
                if (sourceCombo) {
                    categoryCombo = { id: sourceCombo.id }
                    addProgress(`🔗 Using category combo: ${sourceCombo.name}`, 'info')
                }
            } catch (err) {
                addProgress(`⚠️ Could not fetch category combo, using default`, 'warning')
            }
        }
    ...
    const handleCreateMetadata = async () => {
        ...
        try {
            ...
            setCurrentStep(3)
            const categoryCombo = { id: 'bjDvmb4bfuf' } // Use default for now
